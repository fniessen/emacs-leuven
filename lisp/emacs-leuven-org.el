...
(defun leuven--org-update-buffer-before-save ()
  ...
  (when (derived-mode-p 'org-mode)
    ...
    (let (;; Optimize line navigation operations.
          (cache-long-scans nil)        ; Make `forward-line' much faster and
                                        ; thus `org-goto-line', `org-table-sum',
                                        ; etc.
          (flyspell-state (and (boundp 'flyspell-mode) flyspell-mode))
          ;; Preserve undo history (for goto-chg).
          (buffer-undo-list buffer-undo-list))

      ;; Temporarily disable Flyspell to avoid unnecessary spell checks.
