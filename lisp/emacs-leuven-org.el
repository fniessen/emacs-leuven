...
formats (Markdown, HTML, or PDF)."
    ...
    (let* ((orgfile (buffer-file-name))
      ...
      (measure-time-wrapper "Buffer saved" #'save-buffer)
...
Signal a user error when COMPILER-NAME is not supported by this
configuration."
    (pcase compiler-name
      ("pdflatex" "-pdf")
      ("lualatex" "-pdflua")
      ...
      (_
       (user-error
        "[Unsupported LaTeX compiler `%s']" compiler-name))))
...
Prefer `latexmk', which reads the project's `.latexmkrc'.  If `latexmk'
is unavailable, invoke the selected compiler three times so references,
the table of contents, and similar cross-references can stabilize."
    (when (org-export-derived-backend-p backend 'latex)
      (let* ((compiler-name
             ...
             ;; Org export placeholders, expanded when the command is run.
             (latex-file "%f")
             (output-directory "%o"))
         ...
         (if (and latexmk-path latexmk-engine)
             (list
              (format
               ...
               "latexmk -c; latexmk -cd %s -output-directory=\"%s\" \"%s\""
               latexmk-engine
               output-directory
               latex-file))

           ;; Match the important engine flags from `.latexmkrc' when latexmk
           ;; is unavailable.
           (let ((command
                  (format
                   (concat "%s"
    ...
    (let ((cache-long-scans nil)        ; Speed up line navigation in large
      ...
      (unwind-protect
          (progn
            ...
            ;; Remove redundant local tags.
            (boost-org-remove-redundant-local-tags)
